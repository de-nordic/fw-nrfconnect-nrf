 ...
 * @param	time_it_takes	: time [ms] it takes device to complete operation.
 *
...
/**
 * @brief IOCTL handler working at user level

 * @note The system and user level IOCTL handlers use the same IDs for
 * IOCTL operations. Internal implementation should reject syscall level
 * IOCTL operation, by returning -ENOTSUP,  when requested in syscall level handler.
 *
 * @param	dev		: device to perform operation on.
 * @param	id		: IOCTL operation ID
 ...
 * case of other error.
 */
typedef int (*zsai_api_user_ioctl)(const struct device *dev, uint32_t id,
...
/**
 * sys_cal_bitmap is set by driver for each IOCTL to indicate that IOCTL requires
 * syscall level access. This is used for optimization of calls, so when IOCTL
 * does not require syscall level it will be invoked with userspace context.
 * Given IOCTL can not at the same time be syscall level and userspace level.
 * The bitmap setup has been left for device initialization as during initialization
 * the driver may decide whether in given configuration an operation will require
 * higher level or not. For example if we have device that has size pre set at
 * compile time, the size of device could be returned by user lavel IOCTL
 * as it does not required direct device or bus access; on the other side if
 * we have a device that requires run-time query for size, such operation
 * may not be performed without sycall level IOCTL call.
 */
__subsystem struct zsai_driver_api {
	zsai_api_read read;
	zsai_api_write write;
	uint32_t sys_call_bitmap;
	zsai_api_user_ioctl user_ioctl;
 ...
 * TODO
 */
__syscall int zsai_sys_ioctl(const struct device *dev, uint32_t id,
			      const uintptr_t in, uintptr_t in_out);

static inline int z_impl_zsai_sys_ioctl(const struct device *dev, uint32_t id,
					 const uintptr_t in, uintptr_t in_out)
{
	const struct zsai_driver_api *api = ZSAI_API_PTR(dev);

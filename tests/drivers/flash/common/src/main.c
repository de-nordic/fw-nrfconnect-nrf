...
static void *flash_driver_setup(void)
{
	...
	/*
	 * If there is mix of devices then need to get erase_requirement
	 * for device we work with, as we do not know what device we are
	 * working with now.
	 */
	if (FLASH_EVAL_ERASE_REQUIREMENT(true)) {
		const struct flash_parameters *fparams = flash_get_parameters(flash_dev);
		erase_value = fparams->erase_value;
		ebw_required = fparams->flags.erase_requirement;
	...
	if (FLASH_EVAL_ERASE_REQUIREMENT(ebw_required)) {

...
/* No userspace here, this is always userspace call */
static int flash_nrf_user_ioctl(const struct device *dev, uint32_t id, const uintptr_t in,
				uintptr_t in_out)
{
...
static const struct zsai_driver_api flash_nrf_api = {
	...
	.user_ioctl = flash_nrf_user_ioctl,
	.sys_call_bitmap = ZSAI_IOCTL_SYSCALL_BIT(ZSAI_IOCTL_DO_ERASE),
};
